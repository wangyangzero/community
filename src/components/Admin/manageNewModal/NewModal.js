...
import {getNewInfo,addNewInfo,updateNewInfo,deleteNewInfo} from '../../../redux/action/adminHome';
...
class NewModal extends Component {
    ...
    componentDidMount(){
        this.props.dispatch(getNewInfo())
            .then(() => {
                if(!!this.props.adminHome.getNewInfo){
                    this.setState({
                        newInfo: this.props.adminHome.getNewInfo
                    })
                }
            })
    }
    ...
    handleAddNewsOk = e => {
        ...
        this.props.dispatch(addNewInfo({
            href: record.href,
            title: record.title,
    ...
    handleUpdateNewsOk = e => {
        let record = this.props.form.getFieldsValue();
        this.props.dispatch(updateNewInfo({
            id: record._id,
            href: record._href,
            title: record._title,
    ...
    handleDeleteNews = (record) =>{
        this.props.dispatch(deleteNewInfo({
            id: record._id
        }));
    ...
    render() {
        const data = [];
        if(this.state.newInfo.length > 0){
            ...
            for(let i = 0;i < newInfo.length;i++){
                data.push({
                    ...
                    title: newInfo[i].data.title,
                    description: newInfo[i].data.description,
                    avatar: newInfo[i].data.avatar,
                    tag:newInfo[i].data.tag,
                    tagColor: newInfo[i].data.tagColor,
                    content: newInfo[i].data.content,
                })
            }
        }
        const columns=[
